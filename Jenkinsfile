  ...
  stages {
    ...
    stage('dev') {
      ...
      steps {
        sh 'yarn'
        sh 'yarn export'
        sh 'docker build -t registry-vpc.cn-beijing.aliyuncs.com/basicfu/sip-api-ui:${IMAGE_VERSION} .'
        sh 'docker push registry-vpc.cn-beijing.aliyuncs.com/basicfu/sip-api-ui:${IMAGE_VERSION}'
        sh 'curl -d "name=sip-api&set=web.tag=${IMAGE_VERSION}" ${KUBE_TRIGGER_URL}'
      }
    }
  }
  environment {
